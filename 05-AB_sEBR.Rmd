...
if (knitr::is_latex_output()) {
  ...
  out_width_h <- "180mm" # landscape figure
  tbl_font_size <- 10
...
There was also a significant difference in AB magnitude (lag 8 minus lag 3 T2|T1 accuracy) between the sessions: (_F_(`r ICC_ABmag$stats["df","Judges"]`, `r ICC_ABmag$stats["df","Residual"]`) = `r printnum(ICC_ABmag$stats["F","Judges"])`, _p_ `r printp(ICC_ABmag$stats["p","Judges"])`). For most participants, AB magnitude was smaller in the second session than the first (Figure \@ref(fig:fig-retest)B). The average difference in AB magnitude over sessions seemed to be driven by lag 3 performance only (Figure \@ref(fig:fig-retest)A), meaning the AB genuinely decreased with practice.
...
In contrast to AB magnitude, sEBR (Figure \@ref(fig:fig-retest)C), did not differ significantly between sessions (_F_(`r ICC_sEBR$stats["df","Judges"]`, `r ICC_sEBR$stats["df","Residual"]`) = `r round(ICC_sEBR$stats["F","Judges"],3)`, _p_ = `r round(ICC_sEBR$stats["p","Judges"],3)`). We had some concerns that participants would blink less in the second session, because they had been instructed (after the sEBR measurement in session 1) that blinking can cause artifacts in the EEG signal (recorded during task performance). Yet, if anything, median sEBR was slightly higher in the second session (`r printnum(median_sEBR2, digits = 1)`) than the first (`r printnum(median_sEBR1, digits = 1)`). However, we also asked participants whether they had been in an EEG experiment
...
```{r fig-retest, fig.cap='(ref:caption-fig-retest)', results='show', out.width = out_width_h}
...
```{r fig-tDCS-corr, fig.cap='(ref:caption-fig-tDCS-corr)', results='show', out.width = out_width}
...
ABmag_tbl$estimate <- printnum(ABmag_tbl$estimate, gt1=F) # format r-values
ABmag_tbl %>%
  kable(col.names = c("Block", 
        ...
        align = c("l",rep("r",2)),
        escape = FALSE) %>%
  kable_styling(font_size = tbl_font_size) %>%  
...
if (knitr::is_latex_output() | knitr::is_html_output()) {
corr_co_tbl[,c("estimate","r_reliability")] <- printnum(corr_co_tbl[,c("estimate","r_reliability")], gt1 = F) # format r-values
corr_co_tbl[,c("p.value","p")] <- printnum(corr_co_tbl[,c("p.value","p")], gt1 = F, digits = 3) # format p-values
corr_co_tbl %>%
  ...
  add_header_above(setNames(c(1,3,2,1), c(" ",
              ...
              line_break(paste0("Pearson-Filon\ntest", footnote_marker_alphabet(2, double_escape = TRUE)), align = 'c', double_escape = TRUE),
